...
pub struct Bar {
    ...
    /// by sys_update_chord_timing.
    rhythm: Vec<(Duration, Lifetime)>,

    /// Automatically-generated ("managed") rests, managed by sys_update_chord_timing.
    managed: Vec<Entity>,
...
impl Bar {
    ...
    pub fn remove(&mut self, chord: Entity) -> Option<Lifetime> {
        let mut ret = None;

        for (_, entity) in &mut self.rhythm {
            if entity.to_option() == Some(chord) {
                ret = Some(*entity);
    ...
    fn target_managed_count(&self) -> usize {
        ...
        for chord in &self.rhythm {
            if chord.1.is_automatic() {
